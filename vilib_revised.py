...
class Vilib(object):
    ...
    def camera():
        ...
        try:
            ...
            while True:
                ...
                # ----------- draw threshold ---------------
                #thr = 0.7
                #h, w, c = Vilib.img.shape
                #cv2.line(Vilib.img, (int(thr*w), 0), (int(thr*w), h), (0, 255, 255), thickness=5)
                ...
                # draw threshold line <-- newly coded       
                if Vilib.draw_thr_line:
                    h, w, c = Vilib.img.shape
                    cv2.line(
                        Vilib.img,
                        (int(Vilib.thr*w), 0),
                        (int(Vilib.thr*w), h),
                        Vilib.line_color,
                        thickness=Vilib.line_thickness
                        )
    ...
    # newly coded
    @staticmethod
    def show_thr_line(thr=0.7, line_color=(0,255,255), line_thickness=5):
        Vilib.thr = thr
        Vilib.line_color = line_color
        Vilib.line_thickness = line_thickness
        Vilib.draw_thr_line = True
